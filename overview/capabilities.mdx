...
import CompatCallout from '/snippets/compat-callout.mdx';
...
## Already built on another platform?

If your application talks to OpenAI, OpenRouter, ElevenLabs, or Groq today, the **Compatible APIs** serve those protocols directly — point your existing SDK at `/compat/v1` (OpenAI), `/compat/api/v1` (OpenRouter), `/compat/elevenlabs` (ElevenLabs), or `/compat` (Groq), and swap in your Fish Audio key. Model names carry over as aliases; synthesis takes Fish voice IDs in place of vendor preset names. [Migrate to Fish Audio](/developer-guide/getting-started/migration) has the details.

<CardGroup cols={2}>
  <Card title="Migrate to Fish Audio" icon="signs-post" href="/developer-guide/getting-started/migration">
    Point your SDK at a new base URL, pick a Fish voice ID, and keep the rest of your code.
  </Card>

  <Card title="Compatibility" icon="list-check" href="/developer-guide/compat/capabilities">
    The compatibility contract: mappings, limits, and refusals.
  </Card>
</CardGroup>
