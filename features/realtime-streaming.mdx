...
<Info>
  **Streaming from another vendor's SDK?** Fish Audio serves the OpenAI
  Realtime WebSocket at `https://api.fish.audio/compat/v1/realtime` and the
  ElevenLabs `stream-input` WebSocket at
  `https://api.fish.audio/compat/elevenlabs/v1/text-to-speech/{voice_id}/stream-input`
  — swap in your Fish Audio key and
  a Fish voice ID. The OpenRouter and Groq SDKs have no realtime API of their
  own; reach the same socket with the OpenAI SDK. See
  [Migrate to Fish Audio](/developer-guide/getting-started/migration) and the
