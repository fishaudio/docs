...
Fish Audio publishes ready-made [Agent Skills](https://github.com/vercel-labs/skills) that teach your coding agent how to write correct Fish Audio code:

- **`fish-audio-api`** — call the Fish Audio REST and WebSocket APIs directly, without an SDK. Covers authentication, every endpoint in our OpenAPI schema, MessagePack vs JSON vs multipart encoding rules, multi-speaker dialogue, and the WebSocket streaming protocol. Best for `curl`, languages without an official SDK, and edge runtimes.
- **`fish-audio-sdk`** — write code with the official Python (`fish-audio-sdk`) and JavaScript/TypeScript (`fish-audio`) SDKs. Covers install and auth, sync + async Python, the TypeScript client, exact method signatures, model selection, the real exception types, and the Python↔JavaScript naming differences. Best when you've installed an official SDK.
...
<Tabs>
  ...
  <Tab title="Install the SDK skill">
    ```bash
    npx skills add https://docs.fish.audio --skill fish-audio-sdk
    ```

    This installs the SDK skill (for example into `~/.claude/skills/fish-audio-sdk/`). Once installed, ask your agent to "generate speech with the Fish Audio Python SDK" or "transcribe audio with the fish-audio npm package" and it will use the correct client, methods, and error types.
...
<Tip>
  The `skills` CLI works with any agent that uses `SKILL.md` conventions —
  Claude Code, Cursor, Windsurf, Codex, and others. See [`npx skills
  --help`](https://github.com/vercel-labs/skills) for agent-specific install
  flags such as `-a claude-code` or `-a cursor`.
</Tip>

<Note>
  Prefer MCP if you want live documentation search inside your editor. Prefer
  the Agent Skill if you want a self-contained instruction file that works
  offline after install and doesn't rely on a running MCP server.
</Note>
