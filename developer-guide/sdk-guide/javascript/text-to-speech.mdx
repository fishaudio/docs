...
{/* speak-mintlify-hash: fc17eef01159e7d16739b323c5a330a562a917a11e29489e3ccfc436fe8c5841 */}
<Visibility for="humans">
  <AudioTranscript page="javascript-text-to-speech" />
</Visibility>
...
The SDK automatically retries transient failures (`408`, `429`, and `5xx`) with exponential backoff, so you don't need a manual retry loop. Catch the typed errors to handle the rest:
...
try {
  const audio = await fishAudio.textToSpeech.convert({ text: "Your text here" });
  await play(audio);
} catch (err) {
  if (err instanceof UnprocessableEntityError) {
    console.error("Invalid request (422):", err.body?.detail);
  } else if (err instanceof FishAudioTimeoutError) {
    console.error("Request timed out");
  } else if (err instanceof FishAudioError) {
    console.error(err.statusCode, err.body); // 401 bad key, 402 out of credit, 404 voice not found, ...
  } else {
    throw err;
  }
}
...
To tune retries and timeout per call, pass `requestOptions` (the trailing argument):
