...
<Visibility for="humans">
  <AudioTranscript page="javascript-voice-cloning" />
</Visibility>
...
```typescript
import { FishAudioError, UnprocessableEntityError } from "fish-audio";

try {
  await fishAudio.textToSpeech.convert({ text: "Test speech", references: [referenceAudio] });
} catch (err) {
  if (err instanceof UnprocessableEntityError) {
    // 422 — invalid reference audio (wrong format, too short, missing transcript, ...)
    console.error("Invalid reference audio:", err.body?.detail);
