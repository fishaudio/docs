...
<Visibility for="humans">
  <AudioTranscript page="javascript-speech-to-text" />
</Visibility>
...
```typescript
import { FishAudioError, FishAudioTimeoutError } from "fish-audio";

try {
  await fishAudio.speechToText.convert({ audio: createReadStream("audio.mp3") });
} catch (err) {
  if (err instanceof FishAudioTimeoutError) {
    console.error("Request timed out");
  } else if (err instanceof FishAudioError) {
    if (err.statusCode === 413) console.error("Audio file too large (max 20MB)");
    else if (err.statusCode === 400) console.error("Invalid audio format");
    else console.error(err.statusCode, err.body);
  } else {
    throw err;
  }
}
