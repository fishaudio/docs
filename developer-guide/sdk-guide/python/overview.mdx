...
<Visibility for="humans">
  <AudioTranscript page="python-overview" />
</Visibility>
...
HTTP failures raise [`APIError`](/api-reference/sdk/python/exceptions#apierror-objects) or one of its subclasses — [`AuthenticationError`](/api-reference/sdk/python/exceptions#authenticationerror-objects), [`RateLimitError`](/api-reference/sdk/python/exceptions#ratelimiterror-objects), [`NotFoundError`](/api-reference/sdk/python/exceptions#notfounderror-objects) — and everything inherits from [`FishAudioError`](/api-reference/sdk/python/exceptions#fishaudioerror-objects). Invalid request parameters arrive as an `APIError` with `status == 422` (there is no separate `ValidationError` raised at runtime).
