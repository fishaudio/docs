...
Get started with Fish Audio in less than a minute:
...
<Tip>
For chunk-by-chunk processing, use `stream()` which returns an `AudioStream` (iterable). For real-time streaming with dynamic text, see [Real-time Streaming](#real-time-streaming) below.
</Tip>
...
**Instant voice cloning** - Clone a voice on-the-fly using reference audio:
