...
Both models use the [Speech to Text API](/api-reference/endpoint/openapi-v1/speech-to-text). Select `transcribe-1-pro` with the `model` HTTP header for multi-speaker conversation transcription and inline emotion cues. See the [model comparison](/features/speech-to-text#choose-an-asr-model).
