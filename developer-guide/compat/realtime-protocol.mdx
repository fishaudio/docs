...
(`response.output_audio.delta` and the rest of the GA set) — what current
`openai` packages emit, even though the Node module path still says `beta`.
...
it cuts a previous assistant message's audio, and none is retained server-side — so
it is the one client event that sends no acknowledgement back. Any other event
