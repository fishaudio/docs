...
Set the endpoint path from the table and choose **Europe (`eu`)** or **Australia (`au`)**. Both examples below read these settings:

```bash
export SLNG_TTS_PATH="/v1/tts/slng/fish/tts:s2.1-pro"
export SLNG_REGION="eu"
```

Change `SLNG_REGION` to `au` to use Australia. The examples pass this value in the optional `X-World-Part-Override` header; if you omit the header, SLNG selects the region automatically.
...
For voice cloning with inline reference audio, SLNG accepts `references` containing audio bytes and their transcripts through MessagePack requests. See your model's HTTP reference in [Available models](#available-models) for the request schema.
...
curl --fail-with-body --request POST \
  --url "https://api.slng.ai${SLNG_TTS_PATH}" \
  --header "Authorization: Bearer $SLNG_API_KEY" \
  --header "X-World-Part-Override: $SLNG_REGION" \
...
async def tts_stream():
    url = f"wss://api.slng.ai{os.environ['SLNG_TTS_PATH']}"
    headers = {
        "Authorization": f"Bearer {os.environ['SLNG_API_KEY']}",
        "X-World-Part-Override": os.environ["SLNG_REGION"],
    }
