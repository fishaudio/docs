...
The Voice panel selects the voice your agent speaks with (`voice_id`), its speaking language (`speaking_language`: one of the [52 supported languages](/agents/build/voice-language#speaking-language)), and
 whether it speaks expressively (`expressive`, default `false`). See [Voice & language](/agents/build/voice-language) for picking a voice and [Expressive mode](/agents/build/voice-language#expressive-mode) for expressive delivery.
