...
**Speaking language** sets the language for the agent's conversations: one of `en`, `ja`, `zh`, `ko`, `es`, `fr`, `de`, `pt`, `it`, `nl`, `ar`, `hi`, `id`, or `tr` (`voice.speaking_language` on the wire). Every session converses in this language.
