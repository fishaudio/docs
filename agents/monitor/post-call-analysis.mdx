...
A concise recap of the conversation, enabled by default.
...
- **Output language**: fixed per agent, chosen from 13 languages (English, Chinese, Japanese, Korean, Spanish, French, German, Portuguese, Russian, Arabic, Hindi, Indonesian, Turkish). Default is English.


<Note>
  Summaries and rationales are always written in the configured language, even
  when the conversation happened in another one, so downstream systems see
  consistent output.
</Note>
...
When the conversation doesn't contain the information, the field's value is `null` with a rationale explaining why.

### Success criteria

Up to **10 criteria** per agent, each a `name` plus a `description` (up to 500 characters) stating what a successful conversation looks like, such as "The agent resolved the caller's issue or set clear next steps."
